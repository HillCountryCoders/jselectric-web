...
import ServiceToolsTable from "../subComponents/tables/serviceToolsTable";
import ServiceToolsDrawer from "../subComponents/drawers/serviceToolsDrawer";
import "./style.scss";
...
function ServiceTools() {
  ...
  const addTool = (data, cb = null) => {
    axios
      .post(`${apiPath.prodPath}/api/serviceTools/addTools`, data)
      .then((res) => {
  ...
  const handleSearch = (e) => {
    ...
    axios
      .get(`${apiPath.prodPath}/api/serviceTools/${search}&&${searchOpt.value}`)
      .then((res) => {
        setAllTools(res.data.allTools);
        setLoading(false);
      })
  ...
  return (
    <section className={`${poppins.className} employee-wrap`}>
      <div className="add-btn-wrap">
        <h2 className={poppins.className}>Service Tools</h2>
