...
function PartsItems({ data, toolId }) {
  ...
  const addParts = (dataObj) => {
    let url;
    if (editFlag) {
      url = `${apiPath.prodPath}/api/serviceTools/editPartsItems/${toolId}&&${partId}`;
    } else {
      url = `${apiPath.prodPath}/api/serviceTools/addPartsItems/${toolId}`;
    }
    if (editFlag) {
      axios
        .patch(url, dataObj)
  ...
  const deletePart = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `${apiPath.prodPath}/api/serviceTools/deletePartsItems/${toolId}&&${id}`
          )
          .then((res) => {
            loadUser();
            setActionFlag(false);
